...
import com.edu.flinnt.core.PostViewers;
 ...
 */
public class PostViewersFragment extends Fragment {
    ...
    PostViewers mPostViewers;
    ArrayList<PostViewersItems> mPostViewersItems = new ArrayList<PostViewersItems>();
    ...
    public static PostViewersFragment newInstance(String courseID, String courseName, String postID, String viewerPicUrl, String comeFrom, int position) {
        PostViewersFragment fragment = new PostViewersFragment(/*courseID, coursePicture, mCourseNameTxt, courseAllowedRoles*/);
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (null == mPostViewers) {
            if (Helper.isConnected()) {
                mPostViewersRequest = new PostViewersRequest();
                mPostViewers = new PostViewers(mHandler, courseID, postID, postOrContent, currentTab, offset);
                mPostViewers.sendPostViewersRequest(mPostViewersRequest);
                startProgressDialog();
            } else {
        ...
        mPostViewersAdapter.setOnItemClickListener(new PostViewersAdapter.OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                if (Helper.isConnected()) {
                    try {
                        PostViewersItems viewer = mPostViewersItems.get(position);
                        String teachersIDs = "";
                        String studentsIDs = "";
                        int teachersCount = 0;
                        int studentsCount = 0;
                        ...
                        //  PostViewersFragment.this.startActivityForResult(addMessage, PostViewersSendMessageActivity.POST_VIEWRS_FRAGMENT_CALLBACK);
                    } catch (Exception e) {
                        LogWriter.err(e);
                    }
                } else {
    ...
    private void updateViewersList(PostViewersResponse postViewersResponse, int currentTab) {
        ...
        if (mPostViewersAdapter.getItemCount() == 0) {
            ...
            mEmptyTxt.setText(getResources().getString(R.string.no_viewers_msg));
        }else {
    ...
    public void onStart() {
        super.onStart();
        try {
            MyCommFun.sendTracker(getActivity(), "activity="+Flinnt.POST_VIEWERS+"&user="+ Config.getStringValue(Config.USER_ID)+"&course="+courseID+"&post="+postID);
