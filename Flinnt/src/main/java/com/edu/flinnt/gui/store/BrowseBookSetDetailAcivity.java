...
import com.edu.flinnt.adapter.store.BooksetDetailAdapter;
...
import static com.edu.flinnt.R.id.ll_booksetlist;
import static com.edu.flinnt.R.id.review_base_linear;

public class BrowseBookSetDetailAcivity extends AppCompatActivity implements View.OnClickListener,Animation.AnimationListener,YouTubePlayer.OnInitializedListener{
    ...
    private StoreBookSetDetailModel.Data mBrowsableCourse2;
    ...
    private RealmList<com.edu.flinnt.protocol.contentlist.Sections> mSectionList = new RealmList<com.edu.flinnt.protocol.contentlist.Sections>();
    ...
    public static StoreBookSetDetailModel.Data browsableCourse;
    private TextView textCartItemCount;
    private int mCartItemCount;
   //25-01-2019
    private LinearLayoutCompat linear_booksetList;
    private RecyclerView rvBooksetDetailList;
    private BooksetDetailAdapter booksetDetailAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_browse_book_set_detail_acivity);
            ...
            public void handleMessage(Message msg) {
                ...
                switch (msg.what) {
                    case Flinnt.SUCCESS:
                        ...
                        if (msg.obj instanceof StoreBookSetDetailModel) {
                            StoreBookSetDetailModel mCourseViewResponse = (StoreBookSetDetailModel) msg.obj;
...
//                            }
                            mReviewsBaseLinear.setVisibility(GONE);
                            showHideReviewSection();
                            tv_title.setText(mBrowsableCourse2.getBookSetName());
                            updateCourseDetailsNew(mBrowsableCourse2);

                            refreshBooksetDetailView(mBrowsableCourse2.getBooklist());
...
//                            new SuggestedCourses(mHandler, courseId,SuggestedCourses.COURSES_FROM_INSTITUTE).sendSuggestedCoursesRequest();
                                ...
                                Snackbar.make(getWindow().getDecorView().findViewById(android.R.id.content),message,Snackbar.LENGTH_LONG)
                                        ...
                                        .setAction("View Cart",new View.OnClickListener() {
                                            ...
                                            public void onClick(View view) {
                                                startActivity(new Intent(BrowseBookSetDetailAcivity.this,ShoppingCartActivity.class));
                                            }
                                        }).show();
                            }
                    ...
                    case Flinnt.FAILURE:
                        ...
                        try {
                            ...
                            if (msg.obj instanceof CourseViewResponse) {
                                ...
                                Helper.showAlertMessage(BrowseBookSetDetailAcivity.this, "Error", errorMessage, "Close");
                            }
                            ...
                            if (msg.obj instanceof JoinCourseResponse) {
                                ...
                                Helper.showAlertMessage(BrowseBookSetDetailAcivity.this, "Join Course", mJoinCourseResponse.errorResponse.getMessage(), "Close");
                            }
                            if (msg.obj instanceof CheckoutResponse) {
                                CheckoutResponse mCheckoutResponse = (CheckoutResponse) msg.obj;
                                Helper.showAlertMessage(BrowseBookSetDetailAcivity.this, "Join Course", mCheckoutResponse.errorResponse.getMessage(), "Close");
                            }
        ...
        mCourseDescription.sendBookSetDetailRequest();
        ...
        btnAddCart.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                try{
                    ...
                    new CartItems(mHandler,Config.getStringValue(Config.USER_ID),String.valueOf(courseId),mBrowsableCourse2.getBookSetName(),String.valueOf(mBrowsableCourse2.getBookSetId()),String.valueOf(mBrowsableCourse2.getSalePrice()),"1","bookset").sendAddCartRequest2();
                }catch (Exception ex){
                    ex.printStackTrace();
                }
            }
    ...
    private void fillSuggestedCourses(RelatedBookResponse relatedBookResponse) {
        if (null != relatedBookResponse) {
            ...
            if (null != mRelatedBookList && mRelatedBookList.size() > 0) {
                ...
                mJoinedCoursesLinear.setVisibility(GONE);
            }
        }
...
//                }
        ...
        findViewById(R.id.recommended_courses_linear).setVisibility(mJoinedCoursesLinear.getVisibility() == View.VISIBLE || mInstituteCoursesLinear.getVisibility() == View.VISIBLE ? View.VISIBLE : GONE);
    ...
    private void updateBuyerList(BuyerListResponse buyerListResponse, String pictureUrl) {
        ...
        if (courseBuyerAdapter != null) {
            ...
            if (courseBuyerAdapter.getItemCount() < Flinnt.TRUE) {
                mUsersCourseLinear.setVisibility(GONE);
            } else {
            ...
            if (buyerListResponse.getData().getUsers().size() > Flinnt.FALSE) {
                ...
                courseBuyerAdapter.setOnItemClickListener(new CourseBuyerAdapter.OnItemClickListener() {
                    @Override
                    public void onItemClick(View view, int position) {
                        Intent intent = new Intent(BrowseBookSetDetailAcivity.this, BuyerListActivity.class);
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == R.id.rating_linear) {
            ...
            if (!Helper.isConnected()) {
                Helper.showNetworkAlertMessage(BrowseBookSetDetailAcivity.this);
            } else {
                switch (v.getId()) {
...
//                        mReviewsBaseLinear.removeAllViews();
                        ...
                        mReviewCourseLinear.setVisibility(GONE);
                        findViewById(R.id.layout_rating_0).setVisibility(GONE);
                        showHideReviewSection();
    ...
    private void updateCourseDetails(BrowsableCourse browsableCourse) {
        if (null != browsableCourse) {
            ...
            if (!browsableCourse.getVideoUrl().equalsIgnoreCase("")) {
                ...
                mCoursePictureImg.setVisibility(GONE);
                ...
                if (isYoutubeInstalled) {
                    postYouTubePlayerFragment.initialize(Flinnt.DEVELOPER_KEY, BrowseBookSetDetailAcivity.this);
                    mMediaOpenImgbtn.setVisibility(GONE);
                ...
                mPostMediaRelative.setVisibility(GONE);
            }
...
//                mCourseDescriptionTxt.setText(getStrippedString(browsableCourse.getDescription()));
                ...
                mDescriptionLinear.setVisibility(GONE);
            }
            ...
            mCourseDescriptionTxt.post(new Runnable() {
                ...
                public void run() {
                    ...
                    mReadMoreLinear.setVisibility(lineCount >= 10 ? View.VISIBLE : GONE);
                }
            });
            if (browsableCourse.getPublicType().equalsIgnoreCase(Flinnt.COURSE_TYPE_TIMEBOUND)) {
                ...
                mDatetimeTxt.setVisibility(!getStrippedString(browsableCourse.getEventDatetime()).equalsIgnoreCase("") ? View.VISIBLE : GONE);
                mAddressTxt.setVisibility(!getStrippedString(browsableCourse.getEventLocation()).equalsIgnoreCase("") ? View.VISIBLE : GONE);
            } else {
                mDateTimeAddressLinear.setVisibility(GONE);
            }
...
//                }
            ...
            mFinalPriceTxt.setVisibility(GONE);
            mFPriceTxt.setVisibility(GONE);
            mOldPriceTxt.setVisibility(GONE);
            mOPriceTxt.setVisibility(GONE);
...
//            }
    ...
    //09-01-2019 by  vijay
    private void updateCourseDetailsNew(final StoreBookSetDetailModel.Data browsableCourse) {
        if (null != browsableCourse) {
//            if (null == mContentsList && browsableCourse.getIsPublic().equals(Flinnt.ENABLED)) {
//                mContentsList = new ContentsList(mHandler, courseId);
//                mContentsList.setSearchString("");
...
//            }
            ...
            mCourseNameTxt.setText(browsableCourse.getBookSetName());
            mInstituteNameTxt.setText(browsableCourse.getVendorName());
            //BrowseCoursesFragment.cart_count = browsableCourse.getCartTotal();

//            authorListAdapter = new AuthorListAdapter(BrowseBookSetDetailAcivity.this,browsableCourse.getAuthors());
//            mAuthorRvList.setLayoutManager(new LinearLayoutManager(BrowseBookSetDetailAcivity.this));
...
//                    }
//
//                    productDetailListAdapter = new ProductDetailListAdapter(BrowseBookSetDetailAcivity.this, keys, values);
//                    mRVProductAttributeList.setLayoutManager(new LinearLayoutManager(BrowseBookSetDetailAcivity.this));
...
//            }
            ...
            if (!browsableCourse.getDescriptions().get(0).equalsIgnoreCase("")) {

                mCourseDescriptionTxt.setText(Html.fromHtml(browsableCourse.getDescriptions().get(1), null, new MyTagHandler()));

                mRefunPolicyTxt.setText(browsableCourse.getDescriptions().get(0));
        ...
        try {
            if (mContentsResponse.getData().getList().size() > 0) {
                ...
                if (mContentsResponse.getData().getList().size() > 0) {
                    ...
                    mContentLineTxt.setVisibility(GONE);
                    mTotalContentsTxt.setVisibility(GONE);
                    mContainsLinear.setVisibility(GONE);
                    mViewMoreTxt.setVisibility(GONE);
                }
...
//        }
    ...
    private void startNewActivity(BrowsableCourse courseItem) {
        if (Helper.isConnected()) {
            Intent courseDescriptionIntent = new Intent(BrowseBookSetDetailAcivity.this, BrowseCourseDetailActivityNew.class);
    ...
    private void refreshBooksetDetailView(ArrayList<StoreBookSetDetailModel.Booklist> bookData){

        booksetDetailAdapter = new BooksetDetailAdapter(BrowseBookSetDetailAcivity.this,bookData);
        LinearLayoutManager layoutManager = new LinearLayoutManager(BrowseBookSetDetailAcivity.this);
        rvBooksetDetailList.setLayoutManager(layoutManager);
        rvBooksetDetailList.setAdapter(booksetDetailAdapter);
        linear_booksetList.setVisibility(View.VISIBLE);

        //linear_booksetList.getVisibility() ==  GONE ? linear_booksetList.setVisibility(View.VISIBLE) : ;
    ...
    private void findAndInitializeViews() {
        ...
        findViewById(R.id.layout_rating_0).setVisibility(GONE);
        findViewById(R.id.layout_rating_1).setVisibility(GONE);
        findViewById(R.id.layout_rating_2).setVisibility(GONE);
        ...
        linear_booksetList = (LinearLayoutCompat)findViewById(R.id.ll_booksetlist);
        rvBooksetDetailList = (RecyclerView)findViewById(R.id.rv_bookset);
...
//        fl_badge_containers.setOnClickListener(new View.OnClickListener() {
//            @Override
//            public void onClick(View v) {
...
//        });
        ...
        findViewById(R.id.cart_view).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Intent i = new Intent(BrowseBookSetDetailAcivity.this,ShoppingCartActivity.class);
    ...
    private void refreshView() {
        try {
            mContentsDisplayAdapter = new ContentsDisplayAdapter(BrowseBookSetDetailAcivity.this, courseId, courseName, mSectionList);
            GridLayoutManager manager = new GridLayoutManager(BrowseBookSetDetailAcivity.this, 1);
...
//
    ...
    private void sendRequestDialog() {
        try {
            AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(BrowseBookSetDetailAcivity.this);
            // set view
            ...
            mDialogNoteTxt.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    alertDialog.dismiss();
                    Intent profileIntent = new Intent(BrowseBookSetDetailAcivity.this, ProfileActivity.class)
            ...
            alertDialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (TextUtils.isEmpty(notesStr)) {
                        Helper.showAlertMessage(BrowseBookSetDetailAcivity.this, "Course Request", getResources().getString(R.string.course_request_invalide), "CLOSE");
                    } else {
                        if (Helper.isConnected()) {
                            alertDialog.dismiss();
    ...
    private void isAppNotInstalled() {
        mYoutubeFrame.setVisibility(GONE);
        mYoutubeFrame.setVisibility(GONE);
...
//        inflater.inflate(R.menu.browse_course_details_menu, menu);
//        final Menu m = menu;
//        final MenuItem item = menu.findItem(R.id.cart_menu);
//
