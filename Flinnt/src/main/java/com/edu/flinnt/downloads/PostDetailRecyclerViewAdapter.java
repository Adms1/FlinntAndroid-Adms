...
/**
 * Created by flinnt-android-1 on 23/3/17.
 */
...
public class PostDetailRecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public PostDetailRecyclerViewAdapter(Context context) {
    ...
    private void bindData(final AppViewHolder holder, final int position) {
        final AppInfoDataSet appInfoDataSet = mAppInfoDataSets.get(position);
            ...
            public void onClick(View v) {
                if (mListener != null) {
                    if (Helper.isConnected()) {
                        holder.albumProgressBarHint.setVisibility(View.VISIBLE);
                        mListener.onItemClick(v, position, appInfoDataSet);
                    } else {
