...
import com.edu.flinnt.core.PostViewersComments;
 ...
 */
public class PostViewersCommentFragment extends Fragment {
    ...
    PostViewersComments mPostViewersComments;
    ...
    public static PostViewersCommentFragment newInstance(String courseID, String courseName, String postID, String viewerPicUrl, String comeFrom, int position) {
        PostViewersCommentFragment fragment = new PostViewersCommentFragment(/*courseID, coursePicture, mCourseNameTxt, courseAllowedRoles*/);
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (null == mPostViewersComments) {
            mPostViewersRequest = new PostViewersRequest();
            mPostViewersComments = new PostViewersComments(mHandler, courseID, postID, postOrContent,currentTab, offset);
            mPostViewersComments.sendPostViewersCommentsRequest(mPostViewersRequest);
            startProgressDialog();
        }
    ...
    private void updateViewersList(PostViewersResponse postViewersResponse, int currentTab) {
        ...
        if (mPostViewersAdapter.getItemCount() == 0) {
            ...
            mEmptyTxt.setText(getResources().getString(R.string.no_comments_msg));
        } else {
            mEmptyTxt.setVisibility(View.GONE);
        }
        //   mPostViewersAdapter.notifyDataSetChanged();
    }
    ...
    public void onStart() {
        super.onStart();
        try {
            MyCommFun.sendTracker(getActivity(), "PostViewersComment");
